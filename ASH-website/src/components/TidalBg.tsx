...
export function TidalBg() {
  ...
  useEffect(() => {
    ...
    // one dominant octave + a whisper of detail: big calm blobs, no speckle
    const fbm = (x: number, y: number) =>
      0.78 * vn(x, y) + 0.22 * vn(x * 2.1 + 31, y * 2.1 + 17)

    const BASE: [number, number, number] = [246, 245, 241]
    const BLOB: [number, number, number] = [237, 234, 228]
    ...
    const paint = () => {
      ...
      for (let y = 0; y < lh; y++) {
        for (let x = 0; x < lw; x++) {
          const n = fbm(x * 0.026 + t * 0.35, y * 0.042 - t * 0.22)
          const m = sm(Math.min(1, Math.max(0, (n - 0.5) / 0.17)))
